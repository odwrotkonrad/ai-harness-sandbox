...
#[why] ONLY the GCP identity is injected: the sandbox SA key (JSON, terraform-written by iac's auth module) from 1password becomes the pod's ADC; the pod resolves every other secret at runtime from GCP Secrets Manager. attach skips the create-render (keys already on the overlay diff)
