...
typeset configs_ref="$(curl --connect-timeout 30 --retry 10 --retry-delay 30 --retry-all-errors -fsS 'https://gitlab.com/api/v4/projects/konradodwrot%2Fconfigs/repository/commits?path=tools&per_page=1' | sed -n 's/.*"id":"\([0-9a-f]*\)".*/\1/p')"
