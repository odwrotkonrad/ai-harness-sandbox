...
typeset configs_ref="$(curl -fsS 'https://gitlab.com/api/v4/projects/konradodwrot%2Funrestricted%2Fconfigs/repository/commits?path=tools&per_page=1' | sed -n 's/.*"id":"\([0-9a-f]*\)".*/\1/p')"

#[why] etag of the latest che tarball -> CHE_REF: busts the che layer only when che re-published (mirrors the CI jobs)
typeset che_ref=$(curl -fsSI "https://gitlab.com/api/v4/projects/konradodwrot%2Funrestricted%2Fgo-modules/packages/generic/che/latest/che_latest_linux_$(uname -m | sed 's/aarch64\|arm64/arm64/;s/x86_64/amd64/').tar.gz" | tr -d '\r' | awk 'tolower($1)=="etag:"{print $2}')
